...
# Dashboard Startup Script for Day 101
# This script starts the real-time Q-Learning metrics dashboard

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$SCRIPT_DIR"

echo "🚀 Starting Day 101 Q-Learning Dashboard..."
echo ""

# Check if virtual environment exists
if [ ! -d "venv" ]; then
    echo "❌ Virtual environment not found. Running setup_environment.sh first..."
    if [ -f "setup_environment.sh" ]; then
        ...
        echo "❌ setup_environment.sh not found. Please run setup.sh first."
        exit 1
    fi
fi
...
if command -v lsof &> /dev/null; then
    ...
    PORT_IN_USE=$(ss -tulpn | grep :5000 | awk '{print $NF}' | cut -d',' -f2 | cut -d'=' -f2 | head -1)
    if [ ! -z "$PORT_IN_USE" ]; then
        echo "⚠️  Port 5000 is already in use (PID: $PORT_IN_USE)"
        read -p "Do you want to kill the process using port 5000? (y/N): " -n 1 -r
        echo
