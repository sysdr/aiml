...
# Dashboard Startup Script for Day 92
# This script starts the real-time PCA dimensionality reduction metrics dashboard

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$SCRIPT_DIR"

echo "🚀 Starting Day 92 PCA Dimensionality Reduction Dashboard..."
echo ""
...
if command -v lsof &> /dev/null; then
    ...
    PORT_IN_USE=$(netstat -tlnp 2>/dev/null | grep ":5000 " | awk '{print $7}' | cut -d'/' -f1)
    if [ ! -z "$PORT_IN_USE" ]; then
        echo "⚠️  Port 5000 is already in use (PID: $PORT_IN_USE)"
        echo "Killing process using port 5000..."
        kill $PORT_IN_USE
