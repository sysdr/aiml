...
# Dashboard Startup Script for Day 128
# This script starts the real-time activation functions metrics dashboard

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$SCRIPT_DIR"

echo "🚀 Starting Day 128 Activation Functions Dashboard..."
echo ""
...
if [ ! -d "venv" ]; then
    ...
    if [ -f "setup.sh" ]; then
        ...
        echo "❌ setup.sh not found. Please run the main setup.sh first."
        exit 1
    fi
fi
