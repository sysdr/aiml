...
# Dashboard Startup Script for Day 74
# This script starts the real-time feature engineering metrics dashboard

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$SCRIPT_DIR"

echo "🚀 Starting Day 74 Feature Engineering Dashboard..."
echo ""
...
if [ ! -d "venv" ]; then
    ...
    if [ -f "setup_env.sh" ]; then
        ...
        echo "❌ setup_env.sh not found. Please run setup.sh first."
        exit 1
    fi
fi
